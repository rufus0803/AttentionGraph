...
for key in model_state_dict:
    array_model.append(key)

model_bpCL_dict = torch.load(f'model_bpCL.pt',map_location=device)
model_bpCLaf_dict = torch.load(f'model_bpCLaf.pt',map_location=device)

for key in model_bpCL_dict:
    array_1.append(key)

for key in model_bpCLaf_dict:
    array_2.append(key)
# 指定CSV文件路径
csv_file_path = "output.csv"

# 将多个数组合并为一个列表
all_arrays = [array_model, array_1, array_2]

import csv
from itertools import zip_longest


# 使用CSV模块将多个数组的值写入CSV文件的不同列
with open(csv_file_path, mode='w', newline='') as csv_file:
    csv_writer = csv.writer(csv_file)
    for values in zip_longest(*all_arrays, fillvalue=None):
